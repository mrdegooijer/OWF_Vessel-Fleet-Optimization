...
def add_objective_function(model, sets, params, vars):
    ...
    obj_spare_parts_cost = quicksum(holding_cost[s, e] * inventory_level[s, e, p] for s in spare_parts for e in locations for p in periods) + quicksum(order_cost[s] * order_quantity[s, e, p] * base_use[e] for s in spare_parts for e in bases for p in periods)
