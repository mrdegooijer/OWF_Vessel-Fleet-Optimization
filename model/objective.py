...
def add_objective_function(model, sets, params, vars):
    ...
    obj_cost_operations = quicksum(hours_spent[e, v, p, m] * (cost_vessel_operation[v] + cost_technicians * technicians_required_task[m]) for e in locations for v in vessels for p in periods for m in tasks)
    obj_cost_downtime_preventive = quicksum(cost_downtime[p] * time_to_perform_task[m] * task_performed[e, v, p, m] for e in locations for v in vessels for p in periods for m in prev_tasks)
    obj_cost_downtime_corrective = quicksum(cost_downtime[p] * (task_performed[e, v, p, m] * (distance_base_OWF[e]/vessel_speed[v] + 2 * transfer_time[v] + time_to_perform_task[m]) + periods_late[p, m]*24) for e in locations for v in vessels for p in periods for m in corr_tasks) #multiply with 24 because cost is per hour
