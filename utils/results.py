...
def results(model, sets, params, vars, start_time):
    ...
    if model.status == GRB.Status.INFEASIBLE:
        ...
        obj_cost_spare_parts = sum(holding_cost[s, e] * inventory_level[s, e, p].x for s in spare_parts for e in locations for p in periods) + sum(order_cost[s] * order_quantity[s, e, p].x * base_use[e].x for s in spare_parts for e in bases for p in periods)
